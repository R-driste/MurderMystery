...
class App(badge.BaseApp):
    ...
    def send_data_to_host(self, player: Player) -> None:
        self.active_players.append(player)
        self.logger.info(f"Added player: {player}")
        sender_id = badge.info.badge_id()
        payload = b"Hello!"
        packet = sender_id.to_bytes(4, "big") + payload
        badge.radio.send_packet(self.host_id, packet)
        self.logger.info(f"Sent test packet to {dest_id}")
